...
if(isset($_SESSION["login_en"])){
  ...
  $requete = "SELECT * FROM cours where LOGIN_EN = '".$username."' ORDER BY nbvisiteur DESC LIMIT 10";
  $topcours = mysqli_query($conn,$requete);
          ...
          <div class="card">
            <div class="card-header border-0">
              <div class="row align-items-center">
                <div class="col">
                  <h3 class="mb-0">Les 10 cours les plus téléchargés</h3>
                </div>
              </div>
            </div>
              ...
              <table class="table align-items-center table-flush" style="text-align: center">
                <thead class="thead-light">
                  <tr>
                    ...
                    <th scope="col">Nb. Visiteurs</th>
                    <th scope="col">Nb. Telechargements</th>
                    <th scope="col">TD / TP</th>
                  </tr>
                </thead>
                <tbody>
                  <?php   while($reponse = mysqli_fetch_array($topcours)){ ?>
                  <tr>
                    <th scope="row">
                      <?php echo $reponse["TITRE"]; ?>
                    </th>
                    <td>
                      <?php echo round($reponse["nbvisiteur"]/2); ?>
                    </td>
                    <td>
                      <?php echo $reponse["nbtelech"]; ?>
                    </td>
                    <td>
                    <?php
                    $requete = "SELECT count(*) FROM tp where num_c=".$reponse["NUM_C"];
                    $exec_requete = mysqli_query($conn,$requete);
                    $reponses = mysqli_fetch_array($exec_requete);
                    $nbTP = $reponses['count(*)'];

                    $requete = "SELECT count(*) FROM td where num_c=".$reponse["NUM_C"];
                    $exec_requete = mysqli_query($conn,$requete);
                    $reponses = mysqli_fetch_array($exec_requete);
                    $nbTD = $reponses['count(*)'];
                    echo $nbTD." / ".$nbTP;
